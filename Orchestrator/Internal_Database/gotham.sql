...
from (
  select 
    ...
    `view_hp`.`duplicat` AS `hp_duplicat`,
...
select 
  ...
  group_concat(IFNULL(`v`.`hp_duplicat`,'NULL') separator '||||||') AS `hp_duplicat`,
...
from (
  select 
    ...
    group_concat(IFNULL(`view_hp`.`duplicat`,'NULL') separator '||||') AS `hp_duplicat`,
