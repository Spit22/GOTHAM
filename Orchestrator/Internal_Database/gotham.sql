...
select 
  ...
  group_concat(`v`.`link_id` separator '||||||') AS `link_id`,
  group_concat(`v`.`link_nb_hp` separator '||||||') AS `link_nb_hp`,
  group_concat(`v`.`link_nb_serv` separator '||||||') AS `link_nb_serv`,
  group_concat(`v`.`link_ports` separator '||||||') AS `link_ports`,
  group_concat(`v`.`link_tags_hp` separator '||||||') AS `link_tags_hp`,
  group_concat(`v`.`link_tags_serv` separator '||||||') AS `link_tags_serv`,
  group_concat(`v`.`link_created_at` separator '||||||') AS `link_created_at`,
  group_concat(`v`.`link_updated_at` separator '||||||') AS `link_updated_at`,
  group_concat(`v`.`lhs_port` separator '||||||') AS `lhs_port`,
  group_concat(`v`.`serv_id` separator '||||||') AS `serv_id`,
  group_concat(`v`.`serv_name` separator '||||||') AS `serv_name`,
  group_concat(`v`.`serv_descr` separator '||||||') AS `serv_descr`,
  group_concat(`v`.`serv_ip` separator '||||||') AS `serv_ip`,
  group_concat(`v`.`serv_ssh_key` separator '||||||') AS `serv_ssh_key`,
  group_concat(`v`.`serv_ssh_port` separator '||||||') AS `serv_ssh_port`,
  group_concat(`v`.`serv_state` separator '||||||') AS `serv_state`,
  group_concat(`v`.`serv_tags` separator '||||||') AS `serv_tags`,
  group_concat(`v`.`serv_created_at` separator '||||||') AS `serv_created_at`,
  group_concat(`v`.`serv_updated_at` separator '||||||') AS `serv_updated_at` 
from (
  select 
    `view_hp`.`id` AS `hp_id`,
    ...
    group_concat(`Link_Hp_Serv`.`port` separator '||||') AS `lhs_port`,
    group_concat(`view_serv`.`id` separator '||||') AS `serv_id`,
    group_concat(`view_serv`.`name` separator '||||') AS `serv_name`,
    group_concat(`view_serv`.`descr` separator '||||') AS `serv_descr`,
    group_concat(`view_serv`.`ip` separator '||||') AS `serv_ip`,
    group_concat(`view_serv`.`ssh_key` separator '||||') AS `serv_ssh_key`,
    group_concat(`view_serv`.`ssh_port` separator '||||') AS `serv_ssh_port`,
    group_concat(`view_serv`.`state` separator '||||') AS `serv_state`,
    group_concat(`view_serv`.`tags` separator '||||') AS `serv_tags`,
    group_concat(`view_serv`.`created_at` separator '||||') AS `serv_created_at`,
    group_concat(`view_serv`.`updated_at` separator '||||') AS `serv_updated_at` 
  from (
    (
      (
        `view_hp` 
...
select 
  ...
  group_concat(`v`.`link_updated_at` separator '||||||') AS `link_updated_at`,
  group_concat(`v`.`lhs_port` separator '||||||') AS `lhs_port`,
  group_concat(`v`.`hp_id` separator '||||||') AS `hp_id`,
  group_concat(`v`.`hp_name` separator '||||||') AS `hp_name`,
  group_concat(`v`.`hp_descr` separator '||||||') AS `hp_descr`,
  group_concat(`v`.`hp_port` separator '||||||') AS `hp_port`,
  group_concat(`v`.`hp_parser` separator '||||||') AS `hp_parser`,
  group_concat(`v`.`hp_logs` separator '||||||') AS `hp_logs`,
  group_concat(`v`.`hp_source` separator '||||||') AS `hp_source`,
  group_concat(`v`.`hp_state` separator '||||||') AS `hp_state`,
  group_concat(`v`.`hp_port_container` separator '||||||') AS `hp_port_container`,
  group_concat(`v`.`hp_tags` separator '||||||') AS `hp_tags`,
  group_concat(`v`.`hp_created_at` separator '||||||') AS `hp_created_at`,
  group_concat(`v`.`hp_updated_at` separator '||||||') AS `hp_updated_at` 
from (
  select 
    ...
    `view_link`.`updated_at` AS `link_updated_at`,
    group_concat(`Link_Hp_Serv`.`port` separator '||||') AS `lhs_port`,
    group_concat(`view_hp`.`id` separator '||||') AS `hp_id`,
    group_concat(`view_hp`.`name` separator '||||') AS `hp_name`,
    group_concat(`view_hp`.`descr` separator '||||') AS `hp_descr`,
    group_concat(`view_hp`.`port` separator '||||') AS `hp_port`,
    group_concat(`view_hp`.`parser` separator '||||') AS `hp_parser`,
    group_concat(`view_hp`.`logs` separator '||||') AS `hp_logs`,
    group_concat(`view_hp`.`source` separator '||||') AS `hp_source`,
    group_concat(`view_hp`.`state` separator '||||') AS `hp_state`,
    group_concat(`view_hp`.`port_container` separator '||||') AS `hp_port_container`,
    group_concat(`view_hp`.`tags` separator '||||') AS `hp_tags`,
    group_concat(`view_hp`.`created_at` separator '||||') AS `hp_created_at`,
    group_concat(`view_hp`.`updated_at` separator '||||') AS `hp_updated_at` 
  from (
    (
      (
        `view_serv` 
