...
def replace_honeypot_all_link(DB_settings, datacenter_settings, hp_infos):
    ...
    try:
        ...
        honeypots = Gotham_check.check_tags(
            "hp",
            Gotham_link_BDD.get_honeypot_infos(
                DB_settings,
                tags=hp_tags
            ),
            tags_hp=hp_tags
    ...
    honeypots = [hp for hp in honeypots if not(
        hp["hp_state"] == str(state_list[2]).upper()
        or hp["hp_state"] == str(state_list[3]).upper()
        or hp["hp_id"] == hp_infos["hp_id"]
        )
    ]
...
def replace_honeypot_in_link(DB_settings, datacenter_settings,
                             hp_infos, link, duplicate_hp_list=[],
                             new_tags=""):
    '''
    ...
    ARGUMENTS:
        ...
        duplicate_hp_list (list) - optional : list of honeypot that have been
            already duplicated for the replacement of this honeypot
        new_tags (string) - optional : list of new honeypot tag for the link

    Return a dict : "replaced" is a boolean set to true if the honeypot has
    been replaced, false otherwise ; "duplicate_hp_list" is a list of
    honeypot that have been duplicated for the replacement of this honeypot
    '''
    ...
    # Define link honeypot tags either by the new ones if they exist, or by
    # taking those defined in the information of the link and fomat them to be
    # separate by the tag separator
    ...
    if honeypots != []:
        ...
        # Favor already duplicated honeypots to replace this honeypot in other
        # links by reducing a customizable weight
        ...
        #  If already duplicate or no link configured, just edit the link to
        # redirect to the hp
        if (
            honeypots[0]["hp_id"] in duplicate_hp_list
            or honeypots[0]["hp_state"] == str(state_list[0]).upper()
        ) and link["link_id"] not in honeypots[0]["link_id"]:
            # Don't duplicate
...
def decrease_link(DB_settings, datacenter_settings,
                  object_infos, link, type_obj):
    '''
    ...
    ARGUMENTS:
        ...
        type_obj (string) : "hp" or "serv", specify the type of object
            requiring decrease
    ...
    # Check that the number of objects is greater than 1 and can therefore be
    # decrease
    if int(link["link_nb_" + type_obj]) > 1:
        ...
        try:
            if type_obj == "hp":
                # Remove the combinaison Hp-Link in the Link_Hp_Serv table in
                # Internal Database
                Gotham_link_BDD.remove_lhs(
                    DB_settings,
                    id_link=link["link_id"],
                    id_hp=object_infos["hp_id"]
                )
            elif type_obj == "serv":
                # Remove the combinaison Serv-Link in the Link_Hp_Serv table in
                # Internal Database
                Gotham_link_BDD.remove_lhs(
                    DB_settings,
                    id_link=link["link_id"],
                    id_serv=object_infos["serv_id"]
                )
        except Exception as e:
            raise ValueError(e)
        try:
            # Prepare Database modification
            # Define the modifications in Internal Database : decrease the
            # object number in the link
...
def configure_honeypot_replacement(DB_settings, datacenter_settings,
                ...
                with fileinput.FileInput(
                    ...
                    for line in file:
                        if ("  # " + str(old_hp_infos["hp_id"])) in line:
                            ...
                            print(line.replace(
                                "  server " + str(datacenter_settings["hostname"]) + ":" + str(old_hp_infos["hp_port"]) + ";\n", ""),
                                end=''
                            )
                            first_line = False
                        else:
...
def duplicate_hp(DB_settings, honeypot_infos):
    ...
    duplicate_hp_infos = {
        "name": str(name),
        "descr": str(descr),
        "tags": str(honeypot_infos["hp_tags"].replace("||", tag_separator)),
        "logs": str(honeypot_infos["hp_logs"]),
        "parser": str(honeypot_infos["hp_parser"]),
        "port": str(honeypot_infos["hp_port"]),
        "dockerfile": str(encoded_dockerfile.decode("utf-8")), "duplicat": 1
    }
...
def replace_server_in_link(DB_settings, serv_infos,
                           link, new_tags="", already_used=[]):
    '''
    ...
    ARGUMENTS:
        ...
        already_used (list) - optional : list of server that have been
            already used for the replacement of this server
    ...
    # Define link servers tags either by the new ones if they exist, or by
    # taking those defined in the information of the link and fomat them to be
    # separate by the tag separator
    link_tags_serv = tag_separator.join(
        link["link_tags_serv"].split("||")
    ) if new_tags == "" else new_tags

    # Get all servers corresponding to tags
    servers = Gotham_check.check_tags(
        "serv",
        Gotham_link_BDD.get_server_infos(
            DB_settings,
            tags=link_tags_serv
        ),
        tags_serv=link_tags_serv
    )
    ...
    servers = [server for server in servers if not(
        server["serv_state"] == str(state_list[2]).upper()
        or server["serv_state"] == str(state_list[3]).upper()
        or link["link_id"] in server["link_id"]
        or server["serv_id"] in already_used
        or server["serv_id"] == serv_infos["serv_id"]
        )
    ]
    ...
    # Find ports used by server, by going through the list of honeypots linked
    # to the server, either in the link or in the server information
    if "hps" in link.keys():
    ...
    # Find a server in the previous selection with all of ports used by the
    # old one free
    ...
    if servers_same_port != []:
        ...
        # Variable declaration and initialisation to save configs which has
        # already been modified and deployed
        already_deployed = []
        ...
        if "hps" in link.keys():
            ...
            for hp in link["hps"]:
                # If the nginx config for this port has'nt been already
                # deployed
                ...
                # Prepare Database modification
                # Define the modifications in Internal Database : edit the
                # server id in the link
                modifs = {"id_serv": replacement_server[0]["serv_id"]}
                # Define the conditions of  modifications in Internal Database
                conditions = {
                    "id_link": link["link_id"],
                    "id_hp": hp["hp_id"],
                    "id_serv": serv_infos["serv_id"]
                }
                try:
                    # Edit the Link_Hp_Serv table in Internal Database
        ...
        try:
            ...
            Gotham_state.adapt_state(
                DB_settings, serv_infos["serv_id"], "serv")
        except Exception as e:
            logging.error(
                "Error while configuring server state : " + str(e))
    ...
    # If all the honeypots do not use the same exposure ports on the same
    # server
    else:
...
def distribute_servers_on_link_ports(DB_settings, link):
    ...
    # As long as we can equalize the number of servers using the same port on
    # all the ports specified by the link
    while is_possible and cpt < 100:
        ...
        # Variable to count the occurrence of exposure ports declaration and
        # initialisation
        count_exposed_ports = {
        ...
        for server in dsp_link["servs"]:
            ...
            if len(exposed_ports_unique) == 1:
                ...
                count_exposed_ports[str(exposed_ports_unique[0])] = 0 if str(
                    exposed_ports_unique[0]
                ) not in count_exposed_ports.keys() else count_exposed_ports[str(exposed_ports_unique[0])] + 1
            # If all the honeypots do not use the same exposure ports on the
            # same server
            else:
        ...
        honeypots = [{
            key: value for key, value in hp.items() if key != "lhs_port"
        ...
        if port_max_used != port_min_used:
            ...
            for server in dsp_link["servs"]:
                ...
                # If all the honeypots do not use the same exposure ports on
                # the same server
                else:
            ...
            # If some servers match and the selected max port is at least used
            # on two more servers than the least used port
            if servers != [] and count_exposed_ports[str(
                    port_max_used)] - count_exposed_ports[str(port_min_used)] > 1:
                # Take the first server
                servers = [servers[0]]

                # Replace the nginx config to expose the min port selected
                # instead of the max port selected
                try:
                    ...
                    Gotham_SSH_SCP.execute_commands(
                        servers[0]["serv_ip"],
                        servers[0]["serv_ssh_port"],
                        servers[0]["serv_ssh_key"],
                        commands
                    )
                ...
                # Resetting iterable variables to try to redistribute on all
                # ports (in case the previous changes have unlocked other
                # possible changes)
                i = 0
                j = 1

            # If the next most used port isn't the same that the min port
            # selected, and their difference in terms of number of servers
            # exposing them is greater than 1
            elif (
                str(ports_sorted[i + 1]) != port_min_used
                and count_exposed_ports[str(ports_sorted[i + 1])] - count_exposed_ports[port_min_used] > 1
            ):
                # Use the next most used port for the next loop
                i += 1

            # If the next least used port isn't the same that the max port
            # selected, and their difference in terms of number of servers
            # exposing them is greater than 1
            elif (
                str(ports_sorted[-(j + 1)]) != port_max_used
                and count_exposed_ports[port_max_used] - count_exposed_ports[str(ports_sorted[-(j + 1)])] > 1
            ):
                # Use the next least used port for the next loop
                j += 1

            # Can't equalize more the number of servers using the same port on
            # all the ports specified by the link
            else:
                is_possible = False
        else:
