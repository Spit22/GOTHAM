...
def adapt_state(DB_settings, obj_id, obj_type, link_id="",
                check_all=True, replace_auto=True):
    '''
    ...
    if check_all:
        # Check connection to define or not the state to 4th or 3rd of
        # state_list (Default DOWN and ERROR)
        if obj_type == "hp":
            # check docker liveness
            command = 'docker inspect --format="{{json .State}}" $(docker ps -a | grep ' + \
                object_infos["hp_id"] + ' | cut -d " " -f1)'
            try:
                container_state = json.loads(
                    Gotham_SSH_SCP.execute_command_with_return(
                        dc_ip, dc_ssh_port, dc_ssh_key, command)[0])
            ...
            try:
                ssh_test = Gotham_check.check_ssh(
                    object_infos["serv_ip"],
                    object_infos["serv_ssh_port"],
                    object_infos["serv_ssh_key"])
            except ValueError as e:
            ...
            if not(ssh_test) and not(ping_test):
                final_state = str(state_list[3]).upper()
                logging.debug(
            ...
            if final_state == "":
                ...
                try:
                    nginx_running = Gotham_SSH_SCP.execute_command_with_return(
                        object_infos["serv_ip"],
                        object_infos["serv_ssh_port"],
                        object_infos["serv_ssh_key"],
                        command)
                except ValueError as e:
                    error = "Error while trying to execute ssh command for nginx state check on serv (id: " + object_infos["serv_id"] + ") : " + str(
                        e)
                    logging.error(error)
                    raise ValueError(error)
    ...
    if final_state == "":
        if str(object_infos["link_id"]) == str(
                link_id) or str(object_infos["link_id"]) == "NULL":
    ...
    # Update the state
    try:
        state_functions.change_state(
            DB_settings,
            object_infos[obj_type + "_id"],
            obj_type,
            final_state
        )
    except Exception as e:
        raise ValueError(
            "Error while set state to " +
            final_state +
            " : " +
            str(e)
        )

    # If replace_auto is set and object state is DOWN or ERROR, try to replace
    # it
    if (
            replace_auto
            and (final_state == str(state_list[2]).upper()
                 or final_state == str(state_list[3]).upper())
            and str(object_infos["link_id"]) != "NULL"):
        if obj_type == "hp":
            try:
                ...
                error = "Error while trying to replace honeypot (id: " + \
                    object_infos[obj_type + "_id"] + \
                        ") for bad state: " + str(e)
                logging.error(error)
                raise ValueError(error)
            ...
            try:
                ...
                error = "Error while trying to replace server (id: " + \
                    object_infos[obj_type + "_id"] + \
                        ") for bad state: " + str(e)
                logging.error(error)
                raise ValueError(error)
        ...
        if obj_type == "hp":
            object_infos = Gotham_link_BDD.get_honeypot_infos(
                DB_settings,
                id=str(object_infos[obj_type + "_id"])
            )[0]
    ...
    if (
            obj_type == "hp"
            and final_state == str(state_list[0]).upper()
            and int(object_infos[obj_type + "_duplicat"]) == 1):
        try:
            succes = rm_hp.main(
                DB_settings,
                datacenter_settings,
                str(object_infos[obj_type + "_id"])
            )
