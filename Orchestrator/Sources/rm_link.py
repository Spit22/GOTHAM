...
def remove_links_on_servers(DB_settings, result):
    ...
    try:
        lk_display = normalize_display_object_infos(
            result,
            "link",
            "serv"
        )
...
def remove_rsyslog_configuration(server_settings, id_lk):
    ...
    try:
        commands = [
            "rm " + rsyslog_conf_server_remote_path,
            "rm " + remote_lk_log_file_path]
        execute_commands(
            server_settings["serv_ip"],
            server_settings["serv_ssh_port"],
            server_settings["serv_ssh_key"],
            commands
        )
    except Exception as e:
        raise ValueError(e)
    ...
    try:
        for fileToDelete in [rsyslog_conf_server_local_path,
