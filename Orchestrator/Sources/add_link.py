...
def generate_server_rsyslog_conf(orch_ip, orch_rsyslog_port, rulebase_path, id_lk, rsyslog_conf_server_local_path):
    ...
    # rsyslog_conf_server_local_path (string) : intern path of rsyslog server configuration
    ...
    try:
        ...
        rsyslog_conf_file.write('input(Type="imfile" File="/var/log/nginx/'+ str(id_lk) + '.log" Tag="' + str(id_lk) + '")\n')
...
def generate_orchestrator_rsyslog_conf(id_lk, rsyslog_conf_orchestrator_local_path, local_lk_log_file_path):
...
def deploy_rsyslog_conf(servers, orchestrateur_settings, id_lk):
    ...
    # orchestrateur_settings (dict) : all authentication information to connect to orchestrator
    ...
    for server in servers:
        ...
        # Check if required directories on server exists
        rsyslog_conf_server_remote_path_exists = execute_command_with_return(server["serv_ip"],
                                                                                 server["serv_ssh_port"],
                                                                                 server["serv_ssh_key"],
                                                                                 f"[[ -d {rsyslog_conf_server_remote_path} ]] && echo 'OK'")
        remote_rulebase_path_exists = execute_command_with_return(server["serv_ip"],
                                                                  server["serv_ssh_port"],
                                                                  server["serv_ssh_key"],
                                                                  f"[[ -d {remote_rulebase_path} ]] && echo 'OK'")
        if not (rsyslog_conf_server_remote_path_exists == ['OK'] and remote_rulebase_path_exists == ['OK']):
