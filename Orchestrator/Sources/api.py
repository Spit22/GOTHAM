...
DB_settings = {"username": config['internaldb']['username'], "password": config['internaldb']['password'],
               "hostname": config['internaldb']['hostname'], "port": config['internaldb']['port'], "database": config['internaldb']['database']}
dc_ports_list = range(int(config['datacenter']['min_port']), int(
    config['datacenter']['max_port']))
...
orchestrator_settings = {"hostname": orchestrator_ip,
...
def add_lk():
    ...
    try:
        # Normalize infos
        lk_infos_received = {
            "tags_hp": data["tags_hp"], "tags_serv": data["tags_serv"], "ports": data["exposed_ports"]}
    ...
    existingLinks = Gotham_check.check_tags("link", Gotham_link_BDD.get_link_infos(
        DB_settings, tags_hp=tags_hp, tags_serv=tags_serv), tags_hp=tags_hp, tags_serv=tags_serv, mode=True)
    ...
    for exposed_port in exposed_ports_list:
        add_link.generate_nginxConf(
            DB_settings, id, dc_ip, honeypots, exposed_port)
    ...
    # Create lk_infos
    lk_infos = {"id": id, "nb_hp": nb_hp, "nb_serv": nb_serv,
        ...
        if link_infos_received["ports"] != link["link_ports"]:
            try:
                edit_link.edit_ports(
                    DB_settings, datacenter_settings, link_serv_hp, link_infos_received["ports"])
            except Exception as e:
        ...
        if link_infos_received["nb_serv"] != link["link_nb_serv"]:
            try:
                nb_serv = edit_link.edit_nb(
                    DB_settings, datacenter_settings, link_serv_hp, link_infos_received["nb_serv"], "serv")
            except Exception as e:
        ...
        if link_infos_received["nb_hp"] != link["link_nb_hp"]:
            try:
                nb_hp = edit_link.edit_nb(
                    DB_settings, datacenter_settings, link_hp_serv, link_infos_received["nb_hp"], "hp")
            except Exception as e:
