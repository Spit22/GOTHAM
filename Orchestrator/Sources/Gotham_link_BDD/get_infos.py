...
def honeypot(DB_connection, mode=False, id="%", name="%", tags="%", state="%", descr="%", port="%", parser="%", logs="%", source="%", id_container="%"):
    # Frame used arguments with %
    if mode:
        id = normalize_used_arguments(id, "%")
        name = normalize_used_arguments(name, "%")
    ...
    cur = DB_connection.cursor()
    # Execute SQL request
    cur.execute("SELECT * FROM v_hp_full WHERE hp_id LIKE ? AND hp_name LIKE ? AND "+tags_request+" AND hp_state LIKE ? AND hp_descr LIKE ? AND hp_port LIKE ? AND hp_parser LIKE ? AND hp_logs LIKE ? AND hp_source LIKE ? AND hp_id_container LIKE ?", (id,name,state,descr,port,parser,logs,source,id_container))
    # Convert answer to JSON
    row_headers=[x[0] for x in cur.description]
    rv = cur.fetchall()
    json_data=[]
