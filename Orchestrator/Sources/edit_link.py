...
def edit_tags(DB_settings, datacenter_settings, link, tags, type_tag):
    ...
    if type_tag != "hp" and type_tag != "serv":
        logger.error("[edit_link] type_tag field is incorrect")
...
def edit_nb(DB_settings, datacenter_settings, link, nb, type_nb):
    ...
    for object_infos in dsp_link[type_nb + "s"]:
        ...
        try:
            ...
            logger.error(
                "[edit_link] Error while configuring object " + type_nb + " state : " + str(e))
    ...
    if str(nb).lower() == "all" or int(nb) > int(link["link_nb_" + type_nb]):
        ...
        if tags.lower() != "all":
            ...
            try:
                ...
                logger.error(
                    "[edit_link] Error while configuring object state : " + str(e))
        ...
        if type_nb == "hp" and str(nb).lower(
            ...
            for i in range(nb - len(selected_objects)):
                ...
                try:
                    ...
                    logger.error("[edit_link] Error with hp duplication : " + str(e))
                    raise ValueError(e)
        ...
        if type_nb == "hp":
            for exposed_port in exposed_ports_unique:
                ...
                try:
                    ...
                    logger.error(
                        f"[edit_link] Error with nginx conf modification : {nginxRedirectionPath} - " + str(e))
                    raise ValueError(e)
            ...
            for server in servers:
                for honeypot in added_hp:
                    ...
                    try:
                        ...
                        logger.error(
                            "[edit_link] Error while configuring honeypot state : " + str(e))
                ...
                try:
                    ...
                    logger.error(
                        "[edit_link] Error while configuring server state : " + str(e))
        ...
        for del_object in del_objects:
            ...
            if type_nb == "serv":
                try:
                    ...
                    logger.error(
                        f"[edit_link] {link['link_id']} removal on servers failed : {e}")
                    raise ValueError(e)
            try:
                if type_nb == "hp":
...
def edit_ports(DB_settings, datacenter_settings, link, new_ports):
    ...
    for server in dsp_link["servs"]:
        ...
        if len(exposed_ports_unique) == 1:
            ...
            logger.error("[edit_link] Not implemented")
