...
def send_file(hostname, port, ssh_key, file_path, remote_file_path):
    ...
    ARGUMENTS:
        ...
        remote_file_path (string) : path on the remote host we want to put the
            file(s)
    '''
...
def execute_commands(hostname, port, ssh_key, commands):
    ...
    ARGUMENTS:
        ...
        ssh_key (string) : SSH key that allow the orchestrator to connect
            to the remote host
        commands (list) : list of the commands we want to execute on the
            remote server
    '''
    # Init remote_server object
    remote_server = SSH_SCP_classes.GothamServer(hostname, port, ssh_key)
...
def execute_command_with_return(hostname, port, ssh_key, command):
    ...
    ARGUMENTS:
        hostname (string) : hostname of the remote host
        port (string) : SSH port of the remote host
        ssh_key (string) : SSH key that allow the orchestrator to connect to
            the remote host
...
def send_file_and_execute_commands(
        hostname, port, ssh_key, file_path, remote_file_path, commands):
    '''
    ...
    ARGUMENTS:
        ...
        remote_file_path (string) : path on the remote host we want to put
            the file(s)
        commands (list) : list of the commands we want to execute on the
            remote server
    '''
