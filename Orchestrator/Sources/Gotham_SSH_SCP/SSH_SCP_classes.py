...
class GothamServer:
    ...
    def connect(self):
        ...
        logging.info(
            f"[+] SSH connection with the remote server {self.hostname} has just started"
        )
    ...
    def disconnect(self):
        ...
        if self.ssh_session:
            try:
                ...
                logging.info(
                    f"[-] SSH connection with the remote server {self.hostname} has just been closed"
                )
    ...
    def upload_files(self, file_paths, remote_file_path):
        ...
        ARGUMENTS:
            ...
            remote_file_path (string) : path on the remote host we want to
                put the file(s)
        '''
